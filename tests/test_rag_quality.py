...
@pytest.mark.skipif(not has_deepseek_key, reason="DEEPSEEK_API_KEY is not configured for LLM as Judge.")
def test_rag_retrieval_and_answer_quality():
    dataset_path = os.path.join(settings.BASE_DIR, "tests", "data", "golden_dataset.json")
    ...
    try:
        ...
        if count == 0:
            pytest.skip("Chroma collection 'camp_rules' is empty. Skipping RAG quality validation.")
    ...
    assert avg_faithfulness >= 4.0, f"Average RAG Faithfulness ({avg_faithfulness}) is below acceptable standard 4.0"
    assert avg_relevance >= 4.0, f"Average RAG Relevance ({avg_relevance}) is below acceptable standard 4.0"
