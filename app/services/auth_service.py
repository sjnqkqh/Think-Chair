...
def signup_user(db: Session, payload: SignupRequest) -> tuple[User, str]:
    if db.query(User).filter(User.login_id == payload.login_id).first():
        logger.warning(
            "auth.signup.rejected", login_id=payload.login_id, reason="already_exists"
        )
    ...
    logger.info("auth.signup.succeeded", user_id=user.id, login_id=user.login_id)
...
def authenticate_user(db: Session, payload: LoginRequest) -> tuple[User, str]:
    ...
    if not user or not verify_password(payload.password, user.password_hash):
        logger.warning("auth.login.failed", login_id=payload.login_id)
        raise UnauthorizedError("아이디 또는 비밀번호가 올바르지 않습니다.")

    logger.info("auth.login.succeeded", user_id=user.id, login_id=user.login_id)
    return user, create_jwt(str(user.id))
