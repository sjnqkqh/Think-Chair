...
class LLMManager:
    def __init__(self):
        ...
        if api_key and api_key != "your_deepseek_api_key_here":
            logger.info(
                f"Initializing ChatOpenAI with DeepSeek model: {settings.DEEPSEEK_MODEL}"
            )
            self.llm = ChatOpenAI(
            ...
            logger.warning(
                "DEEPSEEK_API_KEY is not set. Falling back to ChatGoogleGenerativeAI (Gemini)."
            )
        ...
        self.prompt_template = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """당신은 제공된 문서(Context)를 바탕으로 질문에 답하는 전문 AI 어시스턴트입니다.
